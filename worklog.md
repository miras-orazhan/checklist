...
---
Task ID: registry-unification-1
Agent: main (Super Z)
Task: 1) Убрать страницу "Сотрудники" везде. 2) Переименовать "Кандидаты" → "Реестр". 3) На странице Реестра — редактируемые поля по ролям. 4) Увольнение — сначала поиск по реестру. 5) Убрать отдельную страницу "Врачи" (всё в Реестре). 6) UX-аудит.

Work Log:
- Бэкенд: добавил PUT /api/candidates/:id/photo — загрузка/замена фото любого кандидата (не только врачей). Обновляет marketing_photo step + doctor_profiles (если врач). Доступ: marketing, account_manager, chief_physician, admin.
- Бэкенд: добавил PATCH /api/candidates/:id/routing-sheet — обновление branchId/positionId на routing sheet. Доступ: admin, hr. При смене должности автоматически обновляется isDoctor flag.
- Фронтенд: убрал /employees и /doctors из навигации и роутов. Убрал импорты Employees и Doctors.
- Фронтенд: переименовал "Кандидаты" → "Реестр" в навигации. Добавил marketing в CANDIDATES_ROLES.
- Фронтенд: полностью переписал pages/candidates/index.tsx — теперь это единый реестр с edit dialog:
  • Список всех кандидатов с ИИН, филиалом, должностью, статусом
  • Клик "Открыть" → диалог с role-based editable полями:
    - Фото (marketing, account_manager, chief_physician, admin) — загрузка/замена/скачивание
    - Профессиональные данные (recruiter, admin) — образование, опыт, сертификаты, email, телефон
    - Филиал и должность (hr, admin) — selects
    - Профиль врача (chief_physician, account_manager, admin) — специализация, стаж, процедуры, био (только для врачей)
  • Все изменения сохраняются через PATCH/PUT эндпоинты параллельно
- Фронтенд: переписал pages/termination/new.tsx — теперь двухшаговая форма:
  • Шаг 1: Поиск сотрудника в реестре по ФИО или ИИН. Показывает результаты с кнопкой выбора. Также есть кнопка "Ввести данные вручную".
  • Шаг 2: Форма с предзаполненными данными (если выбран из реестра) или пустая (если ручной ввод). Все поля editable. Кнопка "← Назад к поиску".
- Тесты через agent-browser:
  • Навигация: "Сотрудники" и "Врачи" убраны, "Реестр" вместо "Кандидаты" ✓
  • Реестр: список всех кандидатов с ИИН, филиалом, должностью ✓
  • Диалог редактирования: админ видит все секции (фото, профессиональные данные, филиал/должность, профиль врача) ✓
  • Увольнение: поиск "Громов" → найден 1 результат → клик → форма предзаполнена (ФИО, филиал, должность, email, ИИН) ✓
  • Кнопка "Ввести данные вручную" → пустая форма ✓
  • Бейджи задач в навигации работают ✓

UX-аудит:
- Плюсы: единый реестр для всех ролей, role-based editing, поиск перед ручным вводом, бейджи задач, сжатие фото
- Минусы: диалог редактирования может быть длинным для врачей (много секций), нет inline-редактирования прямо в таблице
- Рекомендации: для production добавить virtual scrolling для больших списков, debounce на поиск, optimistic updates

Stage Summary:
- Страница "Сотрудники" полностью убрана.
- "Кандидаты" переименованы в "Реестр" — единая страница для всех ролей.
- Страница "Врачи" убрана из навигации (функциональность перенесена в Реестр).
- Каждый участник редактирует свою часть: рекрутер — образование/опыт, маркетолог — фото, HR — филиал/должность, главврач — профиль врача.
- Форма увольнения: сначала поиск по реестру, потом ручной ввод если не найден.
